...
if [[ ! -f "$LINUX_PROGRAMS_FLAG_FILE" ]]; then
	echo "Setting up environment variables and reloading..."
	echo 'source $HOME/.bashrc' >> $HOME/.profile
	echo "alias ll='ls -laH'" >> $HOME/.bashrc
	echo 'export HDF5PATH=/usr/lib/x86_64-linux-gnu/hdf5/serial' >> $HOME/.bashrc
	echo 'export PATH=$HOME/.local/bin:$PATH' >> $HOME/.bashrc
	echo "PLEASE RUN 'source $HOME/.profile' WHEN THIS SCRIPT FINISHES EXECUTION"
...
if [[ ! -f "$LINUX_PROGRAMS_FLAG_FILE" ]]; then
    ...
    libelf-dev software-properties-common htop libbsd-dev
    )
