...
runDir = baseDir+"zsim"
testDir = baseDir+'tests/initial-runs' # this is where all the zsim files are created
cfgsDir = runDir+'/tests/initial-runs'
...
for test in tests:
    ...
    try:
        ...
        famat.write(test + "," + names[names_idx] + "," + str(amat) + "," + str(max(cyc_counts) if len(cyc_counts) > 0 else 0) + "\n")
